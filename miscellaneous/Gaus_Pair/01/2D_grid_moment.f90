...
!integral_P     ==> Normalize P
!x,y min/max     ==> Domain for P
!==============================================================================!
integer::d,Npoints
...
!==============================================================================!
    implicit none 
    double precision::x1(d),x2(d),xij,a,b,Pair_Gaus_NRG,sigma1,sigma2
    double precision,parameter::gama=0.1
    xij=sqrt(sum((x1(:)-x2(:))**2))
    sigma1=c_LJ*(P(x1)*Npoints)**(-1./d)    
    sigma2=c_LJ*(P(x2)*Npoints)**(-1./d)    
    a=-exp(-(xij-sigma1)**2/(gama*sigma1)**2)
    b=-exp(-(xij-sigma2)**2/(gama*sigma2)**2)
    Pair_Gaus_NRG=a+b
  end function Pair_Gaus_NRG
!==============================================================================!
...
!N_MC           ==> Number of MMC steps to distribute points
!N_MC_moments   ==> number of MC moves using Monte carlo to compute the moments 
!N_reg          ==> # grid points for each dimension use regular grid to compute moments
...
!!$  call Moments_MC(Moment,N_MC_moments,xmin,xmax)
!!$  write(*,*) 'Moments by Monte Carlo using N_MC=', N_MC_moments
!!$  write(*,*) Moment(1:9)
!!$  write(*,*) 'Integral of P =', Integral_P
!==============================================================================!
